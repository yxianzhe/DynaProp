...
//入参：原图RGB，输出的mask的位置，图片的name，
//这应该就是整个过程中最耗时的函数
cv::Mat SegmentDynObject::GetSegmentation(const cv::Mat &image,std::string dir, std::string name){
    cv::Mat seg = cv::imread(dir+"/"+name,CV_LOAD_IMAGE_UNCHANGED);
    if(seg.empty()){ //应该是会一直满足的，mask文件夹里并没有关于这一帧的分割结果，因此是读不到这张图的
        PyObject* py_image = cvt->toNDArray(image.clone());//原图转化为numpy格式
        assert(py_image != NULL);
        PyObject* py_mask_image = PyObject_CallMethod(this->net, const_cast<char*>(this->get_dyn_seg.c_str()),"(O)",py_image);//拿到输出的分割结果
        seg = cvt->toMat(py_mask_image).clone();
        seg.cv::Mat::convertTo(seg,CV_8U);//0 background y 1 foreground 拿到mat格式的分割结果
    ...
    return seg;//return了seg分割后的结果
}

void SegmentDynObject::ImportSettings(){
    std::string strSettingsFile = "/home/yxz/Documents/dynamic_robot/dynaslam_ws/src/DynaSLAM/Examples/RGB-D/MaskSettings.yaml";
