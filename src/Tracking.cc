                                ...
                                const std::vector<cv::KeyPoint> &bgd_points, const cv::Mat &bgd_descriptors, const std::vector<float> &dp, const bool bypropagation)
{
    ...
    // mCurrentFrame = Frame(mImGray,imDepth,imMask,_imRGB,timestamp,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth);//这一步里面是用语义分割除去前景的特征点



    // std::chrono::steady_clock::time_point tlight1 = std::chrono::steady_clock::now();
    // LightTrack();
    // std::chrono::steady_clock::time_point tlight2 = std::chrono::steady_clock::now();
    // double t_light_total= std::chrono::duration_cast<std::chrono::duration<double> >(tlight2 - tlight1).count();
    // cout<<"LightTrack takes "<<t_light_total<<" seconds."<<endl;
    imRGBOut = _imRGB;
    ...
    // {
    //     std::chrono::steady_clock::time_point tgeo1 = std::chrono::steady_clock::now();
    //     mGeometry.GeometricModelCorrection(mCurrentFrame,imDepth,imMask);
    //     std::chrono::steady_clock::time_point tgeo2 = std::chrono::steady_clock::now();
    //     double t_geo_total= std::chrono::duration_cast<std::chrono::duration<double> >(tgeo2 - tgeo1).count();
    //     cout<<"Geometry takes "<<t_geo_total<<" seconds."<<endl;
    // }
    // std::chrono::steady_clock::time_point tTrack1 = std::chrono::steady_clock::now();
    mCurrentFrame = Frame(mImGray,imDepth,imMask,imRGBOut,timestamp,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,bgd_points,bgd_descriptors,dp,bypropagation);
    mMask = mask;
    ...
    // std::chrono::steady_clock::time_point tTrack2 = std::chrono::steady_clock::now();
    // double t_Track_total= std::chrono::duration_cast<std::chrono::duration<double> >(tTrack2 - tTrack1).count();
    // cout<<"Track takes "<<t_Track_total<<" seconds."<<endl;


    // if (!mCurrentFrame.mTcw.empty())
    // {
    //     std::chrono::steady_clock::time_point tinpaint1 = std::chrono::steady_clock::now();
    //     mGeometry.InpaintFrames(mCurrentFrame, mImGray, imDepth, imRGBOut, imMask);
    //     std::chrono::steady_clock::time_point tinpaint2 = std::chrono::steady_clock::now();
    //     double t_inpaint_total= std::chrono::duration_cast<std::chrono::duration<double> >(tinpaint2 - tinpaint1).count();
    //     cout<<"Inpaint takes "<<t_inpaint_total<<" seconds."<<endl;
    // }
                                ...
                                const double &timestamp, const std::vector<cv::KeyPoint> &bgd_points, const cv::Mat &bgd_descriptors, const std::vector<float> &dp, const bool bypropagation)
{
    ...
    mCurrentFrame = Frame(mImGray,imDepth,imMask,timestamp,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,bgd_points,bgd_descriptors,dp,bypropagation);
    mMask = mask;
